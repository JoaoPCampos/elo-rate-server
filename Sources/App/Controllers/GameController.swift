...
final class GameController {
    ...
    static func create(_ request: Request) throws -> Future<Game> {
        ...
        return try request.content.decode(Game.self).map({ game -> Game in
    ...
    static func register(_ request: Request) throws -> Future<PlayerStats> {
        ...
        return try request.parameters.next(Game.self).map({ game -> PlayerStats in
                            ...
                            guard let challengerId = challenger.id,
                                ...
                                let gameId = game.id else {
                                    ...
                                    throw Abort(.notFound,
                                                reason: "Either challenger player id, contender player id and or game id, not found.")
                        }
                            
                            return try existsPlayer(withId: challengerId, inGame: game, request).flatMap({ exists -> Future<Bool> in
...
extension GameController {
    ...
    static private func existsPlayer(withId playerId: UUID, inGame game: Game, _ request: Request) throws -> Future<Bool> {
        ...
        return try game.playerStats.query(on: request).all().map({ playerStats -> Bool in
    ...
    static private func updateElos(forMatch match: Match,
                                   inGame game: Game,
                                   asWinner winner: Bool,
                                   _ request: Request) throws -> Future<HTTPStatus> {
        
        return try game.playerStats.query(on: request).all().flatMap({ playerStats -> EventLoopFuture<HTTPStatus> in
            ...
            guard let challengerStats = bothPlayers.filter({ return $0.playerId == match.challengerId }).first,
                ...
                let contenderStats = bothPlayers.filter({ return $0.playerId == match.contenderId }).first else {
                    ...
                    throw Abort(.notFound,
                                reason:"Could not find either challenger player id or contender player id for game \(game.name)")
            }
            ...
            return challengerStats.update(newChallengerElo, winner: !winner).update(on: request).flatMap({ _ -> EventLoopFuture<HTTPStatus> in
                        
                return contenderStats.update(newContenderElo, winner: winner).update(on: request).transform(to: .ok)
            })
        })
    }
